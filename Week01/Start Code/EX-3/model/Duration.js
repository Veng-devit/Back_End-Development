...
export class Duration {
  ...
  // YOUR COMMENT
  /**
   * Subtracts another duration and returns a new Duration (immutable).
   * @param {Duration} other - Another duration to minus.
   * @returns {Duration} A new Duration representing the sub.
   */
  minus = (other) => {
         // YOUR CODE
         return new Duration(this._totalSeconds - other._totalSeconds);
  ...
  toString = () => {
        // YOUR CODE
        const minuth = Math.trunc(this._totalSeconds / 60);
        const second = this._totalSeconds % 60;
        const text = `${minuth}m ${second}s`;
        return text;
  };
}
